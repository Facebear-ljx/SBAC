...
parser.add_argument('--standard', default='standard', help='standard or GAE')
...
class PPO:
    ...
    def learn(self, total_time_step=1e+8):
        ...
        while t_so_far < total_time_step:  # ALG STEP 2
            ...
            batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens, batch_next_obs, rewards, done = self.rollout()  # ALG STEP 3
            ...
            mask = torch.unsqueeze(1 - done, dim=1)
            with torch.no_grad():
                target_v = rewards + self.gamma * torch.multiply(self.critic_net(batch_next_obs), mask)

            A_k = target_v - V.detach()  # ALG STEP 5
            ...
            for _ in range(self.ppo_epoch):  # ALG STEP 6 & 7
                ...
                for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity)), self.batch_size,
                    ...
                    critic_loss = nn.MSELoss()(V, target_v[index])
    ...
    def rollout(self):
        ...
        while t < self.buffer_capacity:
            ...
            while True:
                ...
                batch_next_obs.append(next_state)
                terminal.append(int(done==True))
        ...
        terminal = torch.tensor(terminal, dtype=torch.int).to(device)
        ...
        return batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens, batch_next_obs, rewards, terminal
